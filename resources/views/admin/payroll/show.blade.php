...
<div class="invoice p-3 mb-3">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <div class="row mb-3">
                <div class="col-md-6">
                    <p><strong>Employee:</strong> {{ $payroll->employee->user->name ?? 'N/A' }} (ID: {{ $payroll->employee->employee_id ?? 'N/A' }})</p>
                ...
                <div class="col-md-6 text-md-right">
                    ...
                    @endif
                    <p><strong>Processed By:</strong> {{ $payroll->processor->name ?? 'System' }} on {{ $payroll->updated_at->format('M d, Y H:i') }}</p>
                </div>
            </div>
            ...
            <h5>Data Snapshot (For Auditing):</h5>
                        ...
                        <dl class="row mt-2" style="font-size: 0.9rem;">
                            @foreach($payroll->data_snapshot as $key => $value)
                                <dt class="col-sm-4 text-muted border-bottom pb-1 mb-1">{{ Str::title(str_replace(['_', '-'], ' ', $key)) }}</dt>
                                <dd class="col-sm-8 border-bottom pb-1 mb-1">
                                    @if(is_array($value) || is_object($value))
                                        <pre class="mb-0" style="font-size: 0.85rem; background-color: #f0f0f0; padding: 8px; border-radius: 4px; white-space: pre-wrap; word-break: break-all;">{{ json_encode($value, JSON_PRETTY_PRINT) }}</pre>
                    ...
                    @elseif(is_string($payroll->data_snapshot) && !empty(trim($payroll->data_snapshot)))
                        ...
                        <pre style="font-size: 0.8rem; background-color: #f8f9fa; padding: 10px; border-radius: 5px; white-space: pre-wrap; word-break: break-all;">{{ $payroll->data_snapshot }}</pre>
                    @else
                        <p class="text-muted mt-2"><em>Data snapshot is empty or not in a displayable list format.</em></p>
                    @endif
            @endif
